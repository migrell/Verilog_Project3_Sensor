module top_stopwatch (
    ...
    fsm_controller U_FSM (
        .clk(clk),
        .reset(reset),
        .sw(combined_sw & {3{uart_ready}}),  // UART 초기화 후에만 스위치 신호 전달
    ...
    dist_calculator U_ULTRASONIC (
        ...
        .reset(reset),
        .echo(echo),
        .btn_start((ultrasonic_btn_start | ultrasonic_enable) & uart_ready),  // UART 초기화 후에만 버튼 신호 전달
        .trigger(trigger),
    ...
    dut_ctr U_DUT_CTR (
        .clk(clk),
        .rst(reset),
        .btn_start((w_btn_run & is_temp_humid_mode | temp_humid_enable) & uart_ready),  // UART 초기화 후에만 버튼 신호 전달
