module uart_cu(
   ...
   // 센서 제어 신호
   output reg         ultrasonic_enable, // 초음파 센서 활성화
   output reg         temp_humid_enable, // 온습도 센서 활성화
   
   // 상태 입력
   input  wire        o_run,            // 현재 스톱워치 실행 상태
   input  wire [2:0]  current_state     // 현재 FSM 상태 (3비트로 확장)
);
   ...
   // 추가 명령어 아스키 코드 정의
   localparam CMD_ULTRASONIC = 8'h55;       // 'U'
   localparam CMD_ULTRASONIC_LOWER = 8'h75; // 'u'
   localparam CMD_TEMP_HUMID = 8'h54;       // 'T'
   localparam CMD_TEMP_HUMID_LOWER = 8'h74; // 't'
   ...
   always @(posedge clk or posedge reset) begin
       if (reset) begin
           ...
           w_clear <= 1'b0;
           sw <= 2'b00;
           ultrasonic_enable <= 1'b0;
           temp_humid_enable <= 1'b0;
           timer_start <= 1'b0;
           btn_sel <= 2'b00;
       end else begin
           // 기본값으로 초기화
           ...
           case (state)
               ...
               PROCESS_CMD: begin
                   ...
                   if (run_toggle_pending && is_run_cmd) begin
                       if (current_state[2:1] == 2'b00) begin  // 스톱워치 모드에서만
                   ...
                   // 다른 명령 처리
                       ...
                       case (cmd_reg)
                           ...
                           CMD_HOUR, CMD_HOUR_LOWER: begin
                               if (current_state[2:1] == 2'b01 && !button_active && !button_cooldown) begin  // 시계 모드 & 버튼 비활성 상태
                           ...
                           // 초음파 센서 명령 처리
                           CMD_ULTRASONIC, CMD_ULTRASONIC_LOWER: begin
                               if (current_state[2:1] == 2'b10) begin  // 초음파 모드에서만
                                   ultrasonic_enable <= 1'b1;
                               end
                           end
                           
                           // 온습도 센서 명령 처리
                           CMD_TEMP_HUMID, CMD_TEMP_HUMID_LOWER: begin
                               if (current_state[2:1] == 2'b11) begin  // 온습도 모드에서만
                                   temp_humid_enable <= 1'b1;
                               end
                           end
