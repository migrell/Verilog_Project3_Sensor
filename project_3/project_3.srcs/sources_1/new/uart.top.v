module uart_fifo_top (
    ...
    input wire [2:0] current_state, // 현재 FSM 상태
    ...
    // 센서 제어 신호 추가
    output wire ultrasonic_enable,  // 초음파 센서 활성화
    output wire temp_humid_enable   // 온습도 센서 활성화
);
    // 내부 연결 신호
    ...
    // uart_fifo_top.v 파일 수정 - 모든 포트 연결
    uart_tx U_uart_tx (
        .clk(clk),
        .rst(rst),
        .tick(w_tick),
        ...
        .o_tx_done(w_tx_done),
        .o_tx(tx),
        .o_state(),  // 연결은 하되 사용하지 않음
        .o_bit_count(),  // 연결은 하되 사용하지 않음
...
module uart_tx (
    ...
    always @(*) begin
        ...
        tx_done_next = 1'b0; // tx_done 신호 기본값 변경 (매번 초기화)
